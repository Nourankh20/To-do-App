...
export default function App() {
  ...
  useEffect(() => {
    db.transaction((tx) => {
      tx.executeSql(
        "create table if not exists todo (id integer primary key AUTOINCREMENT not null, Name text, Done int);"
      );
    });
  ...
  const addTask = async () => {
    ...
    db.transaction(async (tx) => {
      tx.executeSql("INSERT INTO todo (Name, Done) VALUES (?,0)", [text], (_, { rows }) => {
  ...
  const deleteTask = async (id: number) => {
    db.transaction(async (tx) => {
      tx.executeSql("DELETE FROM todo where id= ?", [id], (_, { rows }) => {
        console.log(JSON.stringify(rows));
        // onUpdate(" ");
        getTODO();
        // setTask(" ");
  ...
  const getTODO = () => {
    db.transaction((txn) => {
      txn.executeSql("select * from todo", [], (_, { rows }) => {
  ...
  const updateItem = async (id: number) => {
    db.transaction(async(txn) => {
      txn.executeSql("UPDATE todo  SET Done = CASE Done WHEN 0 THEN 1 ELSE 0 END where id = ?", [id]);
      
      txn.executeSql("select * from todo where id=?", [id], (_, { rows }) => {
        ...
        <View style={styles.taskContainer}>
          {task?.map((item: any, index: number) => (
            <View key={index} style={styles.task}>
              <Text style={ item.Done == 0 ?  styles.taskName : styles.taskNameDone}>{item.Name}</Text>
